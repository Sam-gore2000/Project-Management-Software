    ...
    <div className={`layout ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          {sidebarOpen && <div className="nav-section-label">MAIN</div>}

          <NavLink to="/dashboard" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-icon">⊞</span>{sidebarOpen && <span>Dashboard</span>}
          </NavLink>

          <NavLink to="/projects" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-icon">◫</span>{sidebarOpen && <span>Projects</span>}
          </NavLink>

          <NavLink to="/performance" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-icon">📊</span>{sidebarOpen && <span>Performance</span>}
          </NavLink>

          {(isAdmin || isManager) && (
            <>
              {sidebarOpen && <div className="nav-section-label" style={{marginTop:'10px'}}>MANAGEMENT</div>}
              <NavLink to="/users" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
                <span className="nav-icon">👥</span>{sidebarOpen && <span>Add User</span>}
              </NavLink>
            </>
          )}
        </nav>
        ...
        <div className="sidebar-footer">
          ...
          <NavLink to="/profile" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
            ...
            {sidebarOpen && <div className="user-details">
              <div className="user-name">{user?.name}</div>
              <div className="user-role">{user?.role}</div>
            </div>}
          </NavLink>
          <button className="nav-item logout-btn" onClick={() => { logout(); navigate('/login'); }}>
            <span className="nav-icon">⊖</span>{sidebarOpen && <span>Logout</span>}
          </button>
        </div>
      </aside>
